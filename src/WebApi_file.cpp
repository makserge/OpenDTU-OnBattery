...
void WebApiFileClass::init(AsyncWebServer& server, Scheduler& scheduler)
{
    ...
    server.on("/api/file/get", HTTP_GET, [this](AsyncWebServerRequest *request){ 
        this->onFileGet(request); 
    });
    server.on("/api/file/delete", HTTP_POST, [this](AsyncWebServerRequest *request){ 
        this->onFileDelete(request); 
    });
    server.on("/api/file/delete_all", HTTP_POST, [this](AsyncWebServerRequest *request){ 
        this->onFileDeleteAll(request); 
    });
    server.on("/api/file/list", HTTP_GET, [this](AsyncWebServerRequest *request){ 
        this->onFileListGet(request); 
    });

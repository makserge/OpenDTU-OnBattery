...
{
    server.on("/api/i18n/languages", HTTP_GET, [this](AsyncWebServerRequest *request){ 
        this->onI18nLanguages(request); 
    });
    server.on("/api/i18n/language", HTTP_GET, [this](AsyncWebServerRequest *request){ 
        this->onI18nLanguage(request); 
    });
}
